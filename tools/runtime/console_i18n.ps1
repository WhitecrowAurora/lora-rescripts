...
$script:ConsoleTextTable = @{
    en = @{
        ...
        launcher_profile_flashattention_label = 'FlashAttention2'
        launcher_profile_flashattention_description = 'Use the dedicated FlashAttention2 runtime. Supported routes can auto-prefer FlashAttention 2, and you can still force the flash backend per route when needed.'
        ...
        runtime_flashattention = 'FlashAttention2'
        runtime_flashattention_python = 'FlashAttention2 experimental Python'
        ...
        runtime_summary_flashattention = 'Python {python}; Torch {torch}; TorchVision {torchvision}; FlashAttention2 {flashattention}'
...
"@
        ...
        runtime_python_incomplete = '{runtime} is incomplete or corrupted: Python failed the startup/pip check.'
        portable_python_incomplete = 'Portable Python is incomplete or corrupted: Python failed the startup/pip check. Repair or replace the bundled python folder first.'
        venv_python_incomplete = 'Project virtual environment is incomplete or corrupted: Python failed the startup/pip check. Repair or recreate .\\venv first.'
...
"@
        ...
        startup_mode_flashattention = 'FlashAttention2 startup mode enabled. Supported routes can now auto-prefer FlashAttention 2 in this runtime; you can still switch the route config manually if needed.'
        ...
        tag_editor_python_incomplete = 'Tag editor Python is incomplete or corrupted: Python failed the startup/pip check.'
    ...
    zh = @{
        ...
        launcher_profile_flashattention_label = 'FlashAttention2'
        launcher_profile_flashattention_description = '使用 FlashAttention2 专用运行时。支持的训练路线现在可以在这个运行时里自动优先尝试 FlashAttention 2；如有需要也可以继续手动指定后端。'
        ...
        runtime_flashattention = 'FlashAttention2'
        runtime_flashattention_python = 'FlashAttention2 实验运行时 Python'
        runtime_blackwell = 'Blackwell'
...
"@
        ...
        runtime_python_incomplete = '{runtime}不完整或已损坏：Python 启动失败或 pip 不可用。'
        portable_python_incomplete = '便携版 Python 不完整或已损坏：Python 启动失败或 pip 不可用。请先修复或替换内置的 python 目录。'
        venv_python_incomplete = '项目虚拟环境不完整或已损坏：Python 启动失败或 pip 不可用。请先修复或重新创建 .\\venv。'
...
"@
        ...
        startup_mode_flashattention = '已启用 FlashAttention2 启动模式。支持的训练路线现在会在这个运行时里自动优先尝试 FlashAttention 2；如有需要仍可手动切换路线后端。'
        ...
        tag_editor_python_incomplete = 'Tag Editor Python 环境不完整或已损坏：Python 启动失败或 pip 不可用。'
    ...
    ja = @{
        ...
        launcher_profile_flashattention_label = 'FlashAttention2'
        launcher_profile_flashattention_description = 'FlashAttention2 専用ランタイムを使用します。対応ルートではこのランタイム上で FlashAttention 2 を自動優先でき、必要に応じて個別に手動指定もできます。'
        ...
        runtime_flashattention = 'FlashAttention2'
        runtime_flashattention_python = 'FlashAttention2 実験ランタイム Python'
        runtime_blackwell = 'Blackwell'
...
"@
        ...
        runtime_python_incomplete = '{runtime} が不完全または破損しています: Python の起動または pip チェックに失敗しました。'
        portable_python_incomplete = 'ポータブル Python が不完全または破損しています: Python の起動または pip チェックに失敗しました。まず同梱の python フォルダーを修復または置き換えてください。'
        venv_python_incomplete = 'プロジェクト仮想環境が不完全または破損しています: Python の起動または pip チェックに失敗しました。まず .\\venv を修復または再作成してください。'
...
"@
        ...
        startup_mode_flashattention = 'FlashAttention2 起動モードが有効です。対応ルートではこのランタイム上で FlashAttention 2 を自動優先できますが、必要ならルート側の設定で手動切り替えも可能です。'
        ...
        tag_editor_python_incomplete = 'Tag Editor の Python 環境が不完全または破損しています: Python の起動または pip チェックに失敗しました。'
