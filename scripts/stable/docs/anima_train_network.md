...
In the current implementation, `anima_train_network.py` is a thin entrypoint and the standalone training core lives in `scripts/stable/lulynx/anima_lora_trainer.py`.
...
現在の実装では、`anima_train_network.py`は薄いエントリーポイントであり、独立した学習コア本体は`scripts/stable/lulynx/anima_lora_trainer.py`にあります。
...
本文档说明如何在 `sd-scripts` 仓库中使用 `anima_train_network.py` 训练 Anima 的 LoRA (Low-Rank Adaptation) 模型。
当前实现中，`anima_train_network.py` 只是一个薄入口，独立的训练核心位于 `scripts/stable/lulynx/anima_lora_trainer.py`。
...
`anima_train_network.py` now forwards into a standalone Lulynx Anima LoRA core instead of keeping the full implementation inline. Compared with `train_network.py`, the main differences are:
...
`anima_train_network.py`は、現在ではフル実装を直接持たず、独立したLulynx Anima LoRAコアへ処理を渡す構成になっています。`train_network.py`との主な違いは以下の通りです。
...
`anima_train_network.py` 现在不再把完整实现直接塞在脚本里，而是转发到独立的 Lulynx Anima LoRA 核心。与 `train_network.py` 相比，主要差异如下：

* **目标模型：** 面向 Anima DiT 模型。
* **模型结构：** 不再使用 U-Net，而是使用基于 MiniTrainDIT 的 Transformer 结构。文本编码器为 Qwen3-0.6B，同时包含将 Qwen3 嵌入映射到 T5 兼容交叉注意力空间的 LLM Adapter，以及 Qwen-Image VAE（16 通道 latent，8 倍空间下采样）。
* **参数：** DiT 模型路径仍使用通用参数 `--pretrained_model_name_or_path`，Qwen3 文本编码器使用 `--qwen3`，Qwen-Image VAE 使用 `--vae`。LLM Adapter 与 T5 tokenizer 可分别通过 `--llm_adapter_path` 和 `--t5_tokenizer_path` 指定。
* **不兼容参数：** Stable Diffusion v1/v2 的部分参数（如 `--v2`、`--v_parameterization`、`--clip_skip`）不会使用。`--fp8_base` 也不受支持。
* **时间步采样：** 使用与 FLUX 训练相同的 `--timestep_sampling` 选项（`sigma`、`uniform`、`sigmoid`、`shift`、`flux_shift`）。
* **LoRA：** 不再使用按组件拆分的参数，而是使用基于正则表达式的模块选择与按模块控制 rank / 学习率（`network_reg_dims`、`network_reg_lrs`）。模块的排除与包含通过 `exclude_patterns` 和 `include_patterns` 控制。
...
* `--attn_mode=<choice>`
  - Attention implementation to use. Choose from `torch` (default), `xformers`, `sageattn`. `xformers` requires `--split_attn`. This option overrides `--xformers`.
* `--split_attn`
...
* `--attn_mode` - 使用するAttentionの実装。`torch`（デフォルト）、`xformers`、`sageattn`から選択。`xformers`は`--split_attn`の指定が必要です。
