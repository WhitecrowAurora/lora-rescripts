...
- 改进 Anima 训练吞吐与运行时稳定性，优化缓存处理、张量传输路径与运行诊断输出
- 强化 Anima 在 SageAttention 下的实际性能可见性，新增步骤级 profiler 支持与更清晰的后端摘要日志
- 修复多项 Anima 边界问题，包括预览 Prompt、空 Token 路径、旧版文本缓存重建，以及 `save_every_n_epochs=0` 异常
- 新增 Anima 高级调试选项，支持 profiler 统计窗口与 NaN 检查间隔调节

### v1.0.9

- 新增 SafeGuard 与 EMA 训练选项，覆盖主要训练路线，并支持在保存模型时自动导出 EMA 权重
- 修复 Anima 因缺少 VAE 路径导致的启动失败问题，统一了界面字段、预检提示与后端运行时校验
- 改进 SD / SDXL / Flux / SD3 / Lumina / Anima / ControlNet / Textual Inversion 等训练流程的保存一致性与训练稳定性
- 更新项目首页文案、版本元数据与发布说明

### v1.0.8 Beta3

- 新增原生 Anima 训练入口页面与后端集成，支持 Anima LoRA / finetune 工作流
- 新增友好的 Anima LoRA / LoKr 切换、JSON 标签优先读取，以及更顺手的预览 Prompt 流程
- 新增实验性 SageAttention 2 启动脚本，并改进注意力后端侦测与回退逻辑
- 改进阶段分辨率训练稳定性、分布式 / 混合显卡启动处理，以及训练前预检提示

### v1.0.6 Beta2

- 新增实验性 SageAttention 启动支持，可在受支持的 NVIDIA 环境中作为替代 attention 后端
- 改进 Blackwell / RTX 50 / RTX PRO 系列兼容性，减少新显卡环境下的启动失败问题
- 改进 xformers / SDPA / SageAttention 的回退逻辑与运行时问题定位提示
- 修复混合显卡选择问题，强化数据集缓存预检，并补充更多训练选项覆盖
- 新增实验性阶段分辨率训练，并扩展优化器 / 调度器集成

### v1.0.2

- 优化了便携启动流程和项目内 Python 运行时处理
- 修复了标签编辑器启动、依赖兼容和界面主题一致性问题
- 改进了配置解析、SDXL 检查点加载与整体训练兼容性
